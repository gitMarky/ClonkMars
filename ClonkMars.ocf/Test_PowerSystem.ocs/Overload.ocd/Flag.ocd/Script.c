...
private func RefreshPowerNetwork(object network)
{
	...
	for (var link in network.power_producers)
	{
		if (!link)
		{
			continue;
		}
		var actual_network = GetPowerSystem()->GetPowerNetwork(link);
		if (!actual_network || actual_network == network)
		{
			continue;
		}
		// Remove from old network and add to new network.
		network->RemovePowerProducer(link);
		actual_network->AddPowerProducer(link);
	}
	for (var link in network.power_consumers)
	{
		if (!link)
		{
			continue;
		...
		// Remove from old network and add to new network.
		network->RemovePowerConsumer(link);
		actual_network->AddPowerConsumer(link);
	}
	return;
